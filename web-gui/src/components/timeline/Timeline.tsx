...
// 1. Выносим метки ритма в отдельный мемоизированный компонент.
// Он будет перерисовываться ТОЛЬКО если изменится массив onsets или длительность.
...
export const Timeline = () => {
    // Подписываемся ТОЛЬКО на те данные, которые не меняются 60 раз в секунду
    const { onsets, duration, isPlaying, setIsPlaying, setCurrentTime } = useSettingsStore();

    const containerRef = useRef<HTMLDivElement>(null);
    const playheadRef = useRef<HTMLDivElement>(null); // Реф для линии
    const timeTextRef = useRef<HTMLSpanElement>(null); // Реф для текста времени
    ...
    useEffect(() => {
        const unsubscribe = useSettingsStore.subscribe(
            ...
            {
                equalityFn: shallow, // Это заставляет подписку работать правильно с объектами
    ...
    return (
        <div className="h-full w-full bg-sidebar flex flex-col select-none">
            ...
            <div className="flex items-center justify-between px-4 h-10 border-b border-border bg-studio/30">
                <div className="flex items-center gap-4">
                    ...
                    <div className="font-mono text-sm tracking-tighter">
                        {/* Присваиваем реф тексту времени */}
            ...
            <div className="flex-1 relative overflow-hidden p-4">
                ...
                >
                    {/* Мемоизированные статические метки */}
                    <RhythmMarkers onsets={onsets} duration={duration} />

                    {/* Плейхед с рефом для прямого управления */}
                    <div
                        ref={playheadRef}
